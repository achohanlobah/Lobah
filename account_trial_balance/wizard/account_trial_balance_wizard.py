...
class AccountTrialBalanceReport(models.TransientModel):
                        ...
                        if isinstance(new_lines[i]['id'], int):
                            ...
                            if self.dimension_wise_project == 'dimension':
                                ...
                                if self.show_dr_cr_separately:
                                    ...
                                    for j in range(len(projects)):
                                        ...
                                        worksheet.write(row, col,round((list(projects[j].values())[0]),2), mainheaderdata)
                                        col+=1
                                    row+=1
                                else:
                                    col = 7
                            ...
                            if self.dimension_wise_project == 'dimension':
                                ...
                                if self.show_dr_cr_separately:
                                    col = 9
                                    for j in range(len(projects)):
                                        worksheet.write(row, col,round((list(projects[j].values())[0]),2), mainheaders)
                                        col+=1
                                    row+=1
                                else:
                                    col = 7
                                ...
                                if self.show_dr_cr_separately:
                                    col = 9
                                    for j in range(len(months)):
                                        worksheet.write(row, col, round((list(months[j].values())[0]),2), mainheaders)
                                        col+=1
                                    row+=1
                                else:
                                    col = 7
    ...
    def dimension_group_line_calculation(self,lines):
        ...
        for line in lines + [None]:
            ...
            # Exclude lines having no group.
            account = self.get_account(line.get('id'))
            if not account:
                no_group_lines.append(line)
                continue
            codes = [('root', str(line.get('parent_id')) or 'root')] + self.get_account_codes(account)
        ...
        for j in range(0, len(main_list)):
            for k in range(0, len(first_list)):
                if first_list[k]['account_name'] == main_list[j]['account_name'] and first_list[k][
                    'analytic_account_id'] == main_list[j]['analytic_account_id']:
                    main_list[j]['debit'] = main_list[j]['debit'] + first_list[k]['debit']
                    main_list[j]['credit'] = main_list[j]['credit'] + first_list[k]['credit']
                    main_list[j]['balance'] = sum(self.env['account.analytic.line'].search([
                        ("date", ">=", self.date_from),
                        ("date", "<=", self.date_to),
                        ('account_id.name', '=', main_list[j]['analytic_account_id'])]).mapped('amount'))
        ...
        for j in range(0, len(new_list)):
            for k in range(0, len(main_list)):
                if new_list[j]['id'] == main_list[k]['id']:
                    column1.append({main_list[k]['analytic_account_id']: main_list[k]['balance'], 'class': 'number',
                                    'no_format_name': main_list[k]['balance']})
                    ...
                    new_list[j]['level'] = 0
                    new_list[j]['parent_id'] = 'hierarchy_' + str(main_list[k]['group_code']) + str(" ") + str(
                        main_list[k]['group_name'])
                else:
                    column1.clear()
