...
class AccountCustomReport(models.TransientModel):
    ...
    def action_redirect_to_aml_view(self):
        ...
        if self.dimension_wise_project:
            ...
            # TODO: Need to check
            # a[0].analytic_line_ids.filtered(lambda l: l.auto_account_id in FilteredAnalyticAccountIds)
        else:
            self.env.cr.execute("""
                SELECT aml.id
                FROM account_move_line aml
    ...
    def get_general_ledger_detail(self):
        ...
        for Account in self.env['account.account'].browse(AccountIds):
            ...
            MoveLines = [x[0] for x in self.env.cr.fetchall()]
            MoveLines = list(dict.fromkeys(MoveLines))  # Removed duplicated values from list
    ...
    def get_initials(self,account):
        ...
        Account = AccountAccountObj.search([('name','=',account)], limit=1)
        # ExcludeTypes = AccountTypeObj.search([('name','in',['Expenses','Depreciation','Cost of Revenue'])])
        Status = ['posted']
        # InitialBalance, InitialDebit, InitialCredit = None
        if Account.filtered(lambda a: a.account_type in ['expense', 'expense_depreciation', 'expense_direct_cost']):
            return 0,0,0
        if Account:
    ...
    def general_ledger_export_excel(self):
        ...
        for Account in self.env['account.account'].browse(AccountIds):
            ...
            if MoveLineIds:
                for ml in MoveLineIds:
                    AnalyticVals = []
                    # ml_analytic_account = ml.analytic_distribution and AllAnalyticAccounts.filtered(lambda a: a.id == int(list(ml.analytic_distribution)[0]))
                    # Balance = Balance + (ml.debit - ml.credit)
                    if self.account_zero_closing_balance and Balance == 0.0:
                        Vals = {'date': str(ml.date.strftime('%d/%b/%Y')) or '',
                                'move': ml.move_id and ml.move_id.name or '',
                                ...
                                'debit': ml.debit or 0.0,
                                'credit': ml.credit or 0.0,
                                'balance': ml.debit - ml.credit or 0.0,
                                }
                        mainDict[Account.name or '-'].append(Vals)
                        if self.dimension_wise_project:
                            for analytic in DynamicList:
                                analytic_ac_ml = ml.analytic_line_ids.filtered(lambda a: a.account_id.name == analytic)
                                if analytic_ac_ml:
                                    AnalyticVals.append({analytic:analytic_ac_ml.amount or 0.0})
                                else:
        ...
        date_format.borders = linedata.borders
        ...
        for ac in mainDict:
            ...
            for line in mainDict.get(ac):
                row+=1
                row_balance = initial_balance + line.get('balance',0.0)
                initial_balance = row_balance
                totaldebit+= line.get('debit')
                totalcredit+= line.get('credit')
                #THIS IS DATE COLUMN
                worksheet.write(row, 0, line.get('date',False),date_format)
                ...
                if line.get('balance',0.0) == 0.0:
                    ...
                    worksheet.write(row, 5,row_balance ,floatstyle)
