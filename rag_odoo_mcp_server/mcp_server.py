...
class OdooPostgresBackend:
    ...
    def list_companies(self) -> List[Dict[str, Any]]:
        """List all companies from res_company."""
        self._ensure_conn()
        with self._conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("""
                SELECT c.id, c.name,
                       rc.name AS currency,
                       p.name AS parent_company
                FROM res_company c
                LEFT JOIN res_currency rc ON c.currency_id = rc.id
                LEFT JOIN res_company p ON c.parent_id = p.id
                ORDER BY c.id
            """)
            rows = cur.fetchall()
        return [dict(r) for r in rows]
...
TOOL_DEFINITIONS = [
    ...
    types.Tool(
        name="list_companies",
        description="List all companies configured in Odoo (res.company). Returns id, name, currency, and parent company. Use to discover available companies for filtering queries.",
        inputSchema={"type": "object", "properties": {}},
    ),
]
...
def _format_list_companies(result: List[Dict]) -> str:
    lines = [f"Companies ({len(result)}):", ""]
    for c in result:
        parent = f" (parent: {c['parent_company']})" if c.get('parent_company') else ""
        lines.append(f"  - id={c['id']}  {c['name']}  [{c.get('currency', '?')}]{parent}")
    return "\n".join(lines)
...
def create_app(backend: OdooPostgresBackend) -> Starlette:
    ...
    async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[types.TextContent]:
        dispatch = {
            ...
            "list_companies": (backend.list_companies, _format_list_companies),
        }
